...
complete_data = pd.DataFrame()  # Initialize complete_data DataFrame
...
for id_value in tqdm(id_values, desc="Processing IDs"):
    ...
    try:
        ...
        # Append to complete_data DataFrame
...
try:
    # Append to CSV file after loop completion
    complete_data.to_csv('complete_output.csv', index=False)

    # Append to SQL database after loop completion
    with engine.begin() as connection:  # Use a transaction context
        complete_data.to_sql(name='sensor_data', con=connection, if_exists='append', index=False)
